...
class ReportStockCardReportXlsx(models.AbstractModel):
    ...
    def _stock_card_report(self, wb, ws, ws_params, data, objects, product):
        ...
        row_pos = self._write_line(
            ...
            render_space={
                "date_from": "" if not objects.date_from else objects.date_from.strftime('%d %B %Y'),
                "date_to": "" if not objects.date_to else objects.date_to.strftime('%d %B %Y'),
        ...
        for line in product_lines:
            ...
            row_pos = self._write_line(
                ...
                col_specs_section="data",
                render_space={
                    "date": "" if not line.date else line.date.strftime('%d %B %Y'),
