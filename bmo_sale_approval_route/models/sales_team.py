...
class ApprovalSalesTeam(models.Model):
    ...
    approver_ids = fields.One2many(
        comodel_name="sales.team.approver", inverse_name='team_id', string='Approvers')
...
class SalesTeamApprover(models.Model):
    _name = "sales.team.approver"
    _description = "Sales Team Approver"
    _order = 'sequence'
    _rec_name = 'user_id'
    ...
    team_id = fields.Many2one(
        comodel_name='approval.sales.team', string='Team', required=True, ondelete='cascade')
    user_id = fields.Many2one(
    ...
    lock_amount_total = fields.Boolean(
        string="Lock Amount Total", default=True, help="Prevent changes of amount total if Sales approved by this user")
...
class SaleOrderApprover(models.Model):
    ...
    _inherit = "sales.team.approver"
    _description = "Sales Approver"

    team_approver_id = fields.Many2one(
        comodel_name='sales.team.approver', string='Sales Team Approver', ondelete='set null')
    ...
    state = fields.Selection(
        selection=[
            ...
            ('approved', 'Approved'),
            ('rejected', 'Rejected'),
        ], string='Status', readonly=True, required=True, default='to_approve')
