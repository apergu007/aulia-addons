...
class PurchaseRequestTeam(models.Model):
    ...
    company_id = fields.Many2one(
        comodel_name='res.company', string='Company', required=True, index=True, default=lambda self: self.env.company.id)
    lock_amount_total = fields.Boolean(
        string="Lock Amount Total", default=True, help="Prevent changes of amount total if approval route generated")
    approver_ids = fields.One2many(
        comodel_name="purchase.request.team.approver", inverse_name='team_id', string='Approvers')
...
class PurchaseRequestTeamApprover(models.Model):
    _name = "purchase.request.team.approver"
    ...
    team_id = fields.Many2one(
        comodel_name='purchase.request.team', string='Team', required=True, ondelete='cascade')
    user_id = fields.Many2one(
        comodel_name='res.users', string='Approver', required=True)
    role = fields.Char(
        'Role/Position', required=True, default="Approver")
    ...
    lock_amount_total = fields.Boolean(
        string="Lock Amount Total", default=True, help="Prevent changes of amount total if PO approved by this user")
...
class PurchaseRequestApprover(models.Model):
    ...
    _inherit = "purchase.request.team.approver"
    _description = "PO Approver"

    team_approver_id = fields.Many2one(
        comodel_name='purchase.request.team.approver', string='PO Team Approver', ondelete='set null')
    order_id = fields.Many2one(
        comodel_name='purchase.request', string='Order', required=True, ondelete='cascade')
