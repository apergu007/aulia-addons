...
class InventoryStockMovementReport(models.TransientModel):
    ...
    report_up_to_certain_date = fields.Boolean(
        string="Date upto",
        help="Report should be generated up to this specific date")
    up_to_certain_date = fields.Date(
        string="Movements Upto",
        help="Specifies the exact date up to which the inventory movements "
    ...
    def action_excel(self):
        ...
        return {
            ...
            'data': {'model': 'inventory.stock.movement.report',
                      'options': json.dumps(
                         data, default=fields.Date.to_string),
                     'output_format': 'xlsx',
